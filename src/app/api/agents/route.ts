...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      skills = [], config = {}, custom_instructions = '',
    ...
    const defaultConfig = {
      temperature: 0.7, max_tokens: 1024, language: 'es',
      greeting_message: '',
    ...
    const agent = await createAgent({
      id: uuidv4(), name, description, channel,
      system_prompt, skills, config: defaultConfig, is_active: false,
    })
